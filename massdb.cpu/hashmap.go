...
// |   41     |         1       | Type(A/B)   | HashMap(A/B), 1 for A, 2 for B |
...
func createMapFile(filePath string, typ MapType, bl int, pubKey *pocec.PublicKey) (*os.File, error) {
	...
	// TODO: Maybe expand file to actual size?
...
func (hm *HashMap) UpdateCheckpoint() error {
	...
	_, err := hm.data.WriteAt(checkpointByte[:], PosCheckpoint)
	return err
}
...
func NewHashMapA(rootDir string, bl int, pubKey *pocec.PublicKey) (*HashMapA, string, error) {
	pubKeyString := hex.EncodeToString(pubKey.SerializeCompressed())
	pathA := strings.Join([]string{pubKeyString, strconv.Itoa(bl), "a"}, "_") + PlotTempFileSuffix
	pathA = filepath.Join(rootDir, pathA)
	var err error
	if err = CreateHashMap(pathA, MapTypeHashMapA, bl, pubKey); err != nil && err != massdb.ErrDBAlreadyExists {
		return nil, "", err
	}
	defer func() {
		if err != nil {
	...
	if err = hmA.data.Truncate(PosProofData); err != nil {
		return nil, "", err
	}
	return hmA, pathA, nil
}
